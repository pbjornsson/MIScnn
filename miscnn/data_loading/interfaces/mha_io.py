...
"""
class MHA_interface(Abstract_IO):
    ...
    def load_image(self, index):
        ...
        vol_data, header = load(os.path.join(img_path, "imaging.mha"))
        # Transform NIFTI object to numpy array
        #vol_data = vol.get_fdata()
        # Save spacing in cache
        #self.cache[index] = vol.affine
        self.cache[index] = header.get_voxel_spacing()
        # Return volume
        return vol_data
    ...
    def load_segmentation(self, index):
        ...
        seg_data, seg_header = load(os.path.join(seg_path, "segmentation.mha"))
        # Transform NIFTI object to numpy array
        #seg_data = seg.get_fdata()
        self.cache[index] = seg_header.get_voxel_spacing()
        # Return segmentation
        return seg_data
    ...
    def load_details(self, i):
        ...
        vxl_spc = self.cache[i]
        spacing = np.array(vxl_spc)
    ...
    def save_prediction(self, pred, index, output_path):
        ...
        save(pred, os.path.join(output_path, pred_file))
